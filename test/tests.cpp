...
/*
F18FS * fs_format(const char *const fname);
    1   Normal
    2   NULL
    3   Empty string
F18FS *fs_mount(const char *const fname);
    1   Normal
    2   NULL
    3   Empty string
int fs_unmount(F18FS *fs);
    1   Normal
    2   NULL
*/
TEST(a_tests, format_mount_unmount) {
    const char *test_fname = "a_tests.F18FS";
    F18FS *fs = NULL;
    // FORMAT 2
...
TEST(b_tests, file_creation_one) {
    ...
    const char *test_fname = "b_tests_normal.F18FS";
    F18FS *fs = fs_format(test_fname);
    ASSERT_NE(fs, nullptr);
...
TEST(b_tests, file_creation_two) {
    ...
    const char *test_fname = "b_tests_full_table.F18FS";
    F18FS *fs            = fs_format(test_fname);
    ASSERT_NE(fs, nullptr);
    ...
    // That's annoying.
    // AND NONE OF THAT IS RIGHT BECAUSE NOW WE ONLY HAVE 15 PER DIRECTORY OH MAN AND DIR BLOCKS CONTAIN 15 NOW
    // 255 inodes, we need to make ~18 folders to contain all that
...
TEST(c_tests, open_close_file) {
    vector<const char *> filenames{
        ...
        "more/bad_req",
        "/folder/withfilethatiswayyyyytoolongwhydoyoumakefilesthataretoobigEXACT!", "/", "/mystery_file"};
    const char *test_fname = "c_tests.F18FS";
    ASSERT_EQ(system("cp b_tests_normal.F18FS c_tests.F18FS"), 0);
    F18FS *fs = fs_mount(test_fname);
    ASSERT_NE(fs, nullptr);
    int fd_array[256] = {-1};
    // OPEN_FILE 1
...
TEST(f_tests, get_dir) {
    vector<const char *> fnames{
        ...
        "more/bad_req",
        "/folder/withfilethatiswayyyyytoolongwhydoyoumakefilesthataretoobigEXACT!", "/", "/mystery_file"};
    const char *test_fname = "f_tests.F18FS";
    ASSERT_EQ(system("cp c_tests.F18FS f_tests.F18FS"), 0);
    F18FS *fs = fs_mount(test_fname);
    ASSERT_NE(fs, nullptr);
    // FS_GET_DIR 1
...
TEST(d_tests, write_file_fill) {
    ...
    vector<const char *> fnames{"/file_a", "/file_b", "/file_c", "/file_d"};
    const char *test_fname = "d_tests_full.F18FS";
    F18FS *fs = fs_format(test_fname);
    ASSERT_NE(fs, nullptr);
    uint8_t large_eight_five_b_seven[1024 * 3];
    memset(large_eight_five_b_seven, 0x88, 888);
	...
	// total blocks				65536
	// eaten by free bitmap		16
	// eaten by overhead		34
	// one for root dir			1
	// direct inodes			6
	// indirect inodes			1 + 256
	// how many left for double indirect inodes?
	// 65536 - 16 - 34 - 1 - 6 - (1+256) = 65222
	// Okay, the double indirect block eats one more,
	// 65222 - 1 = 65221
	// Got the pattern after trying some numbers!
	// 65221 = 257 * 253 + 1 + 199		careful about the calculations here!
	// so take 254 double indirects, and the last double indirect pointer points to a block which only 199 addresses are used
    ...
    // According to my investigation, there's 199 blocks left
    ASSERT_EQ(fs_write(fs, fd, giant_data_hunk, 512 * 256), 512 * 199);
    delete[] giant_data_hunk;
    // While I'm at it...
    // FS_CREATE 21
    ASSERT_EQ(fs_create(fs, fnames[1], FS_DIRECTORY), 0);
    // And might as well check this. We don't accept dir and file with the same name
...
TEST(e_tests, remove_file) {
    ...
    const char *(test_fname[2]) = {"e_tests_a.F18FS", "e_tests_b.F18FS"};
    ASSERT_EQ(system("cp d_tests_full.F18FS e_tests_a.F18FS"), 0);
    ASSERT_EQ(system("cp c_tests.F18FS e_tests_b.F18FS"), 0);
    F18FS *fs = fs_mount(test_fname[1]);
    ASSERT_NE(fs, nullptr);
    // FS_REMOVE 10
...
TEST(g_tests, seek) {
    ...
    ASSERT_EQ(system("cp d_tests_full.F18FS g_tests.F18FS"), 0);
    F18FS *fs = fs_mount(test_fname);
    ASSERT_NE(fs, nullptr);
...
TEST(h_tests, read) {
    ...
    // FS_READ 6
    ASSERT_EQ(fs_seek(fs, fd, (256 + 6) * 512, FS_SEEK_SET), (256 + 6) * 512);
    nbyte = fs_read(fs, fd, &write_space, 1024);
    ASSERT_EQ(nbyte, 1024);	
    ASSERT_EQ(memcmp(write_space, large_eight_five_b_seven, 512), 0);
    ASSERT_EQ(memcmp(write_space + 512, six_e, 512), 0);
    score += 15;
    // FS_READ 7
    ...
    ASSERT_EQ(fs_seek(fs, fd, 0, FS_SEEK_CUR), 519 * 512);
    // FS_READ 11
    ASSERT_EQ(fs_seek(fs, fd, 98675309, FS_SEEK_CUR), 33397248);
    ASSERT_EQ(fs_seek(fs, fd, -500, FS_SEEK_END), 33396748);
    nbyte = fs_read(fs, fd, write_space, 1024);
    ASSERT_EQ(nbyte, 500);
    ASSERT_EQ(memcmp(write_space, six_e, 500), 0);
    // did you mess up the position?
    ASSERT_EQ(fs_seek(fs, fd, 0, FS_SEEK_CUR), 33397248);
    fs_unmount(fs);
    score += 20;
}
...
TEST(i_tests, move) {
    vector<const char *> fnames{
        ...
        "/folder/withfilethatiswayyyyytoolongwhydoyoumakefilesthataretoobigEXACT!", "/", "/mystery_file"};
    const char *test_fname = "g_tests.F18FS";
    ASSERT_EQ(system("cp c_tests.F18FS g_tests.F18FS"), 0);
    F18FS *fs = fs_mount(test_fname);
    ASSERT_NE(fs, nullptr);
